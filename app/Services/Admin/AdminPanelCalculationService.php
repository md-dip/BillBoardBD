...
class AdminPanelCalculationService
{
    ...
    public function revenueSummary(): array
    {
        ...
        $listingFees = (float) $rows->sum('listing_fees');

        $totals = [
            // Total revenue is the sum of all payments received, including both booking payments and listing fees.
    ...
    /**
     * This block's own copy of the ledger query
     * @return Collection<int, array<string, mixed>>
     */
    private function fetchCommissionLedger(): Collection
    {
        $bookingPayments = DB::table('payments')
            ->join('bookings', 'bookings.id', '=', 'payments.booking_id')
            ->join('billboards', 'billboards.id', '=', 'bookings.billboard_id')
    ...
    /**
     * Every single transaction the platform has earned a cut
     *
    ...
    {
        $transactions = $this->fetchCommissionLedger()->sortByDesc('earned_at')->values();

        $bookingGross = (float) $transactions->where('type', '!=', 'listing_fee')->sum('amount');
        $bookingCut = (float) $transactions->where('type', '!=', 'listing_fee')->sum('platform_cut');
        $listingCut = (float) $transactions->where('type', 'listing_fee')->sum('platform_cut');

        $totals = [
            'count' => $transactions->count(),
            'total_revenue' => round((float) $transactions->sum('amount'), 2),
...
// What the platform actually keeps: booking commission + listing fees.
            'platform_commission' => round($bookingCut + $listingCut, 2),
    ...
    public function settledPaymentsQueryForOwner(int $ownerId)
    {
        return Payment::query()
            ...
            ->whereHas(
                'booking',
                fn ($q) => $q->whereIn('status', self::OWNER_PAYABLE_BOOKING_STATUSES)
    ...
    {
        return round((float) $this->settledPaymentsQueryForOwner($owner->id)->sum('owner_payable'), 2);
    }
    ...
    public function dashboardSummary(): array
    {
        return [
            ...
            'payable_to_owners' => round((float) $this->payableToAllOwners()->sum('amount'), 2),

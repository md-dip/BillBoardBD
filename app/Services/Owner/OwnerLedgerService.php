...
/**
 * Every payment collected on an owner's boards - what the "Revenue (BDT)" tile
 * on their dashboard is made of - and where each one's earnings have got to
 * since.
 *
 * Lifted out of Owner\ReportController so the Transactions page and the
 * BillboardBD Assistant read one ledger. An owner who asks the assistant "how
 * much did the Banani board earn me?" must get the figure their own dashboard
 * would show; two implementations would drift the first time either changed.
 *
 *   advance - the client pays it to submit the request, but admin or this
 *             owner rejecting refunds it in full, so it only counts once the
 *             booking has cleared BOTH approvals.
 *   balance - only payable after that point and never refundable, so it counts
 *             the moment it is paid.
 *
 * Board listing fees are not here on purpose: that is money the owner PAID the
 * platform to list a board, not money a board earned them.
 */
class OwnerLedgerService
{
     ...
     *
     * Deliberately this service's own copy rather than a shared constant: the
     * owner's ledger answers a different question from the admin's (what did MY
     * boards earn ME, with no listing fees and no platform-wide totals), and
     * keeping it self-contained means an admin-side change can never move an
     * owner's numbers by accident.
     */
     ...
     */
    public const PAYABLE_BOOKING_STATUSES = ['paid_in_full', 'pending_proof_review', 'active'];
     ...
     */
    public const AWAITING_ADMIN_STATUS = 'pending_proof_review';

    /**
     * The owner's full ledger.
     *
     ...
     *
     * The optional filters are for the assistant, which slices the same rows to
     * answer "which board did best last quarter". Passing none reproduces the
     * Transactions page byte for byte.
     *
     * @return array{transactions: Collection<int, array<string, mixed>>, totals: array<string, mixed>}
     */
    public function forOwner(
        int $ownerId,
        ?int $billboardId = null,
        ?string $from = null,
        ?string $to = null,
    ): array {
        $payments = DB::table('payments')
            ...
            ->when($billboardId !== null, fn ($query) => $query->where('billboards.id', $billboardId))
            ->select(
        ...
        $transactions = $payments
            ...
            // Date filtering happens here rather than in SQL so it lands on the
            // same `earned_at` the rows are labelled with - the COALESCE of
            // paid_at and created_at, not whichever column the query picked.
            ->when($from !== null, fn (Collection $rows) => $rows->filter(
                fn (array $row) => Carbon::parse($row['earned_at'])->gte(Carbon::parse($from)->startOfDay())
            ))
            ->when($to !== null, fn (Collection $rows) => $rows->filter(
                fn (array $row) => Carbon::parse($row['earned_at'])->lte(Carbon::parse($to)->endOfDay())
            ))
    ...
    /**
     * The same ledger rolled up per billboard - what answers "which of my
     * boards earned the most?" without handing the model every payment row.
     *
     * @return array<int, array<string, mixed>>
     */
    public function perBillboard(
        int $ownerId,
        ?int $billboardId = null,
        ?string $from = null,
        ?string $to = null,
    ): array {
        return $this->forOwner($ownerId, $billboardId, $from, $to)['transactions']
            ->groupBy('billboard_id')
            ->map(fn (Collection $rows, $id) => [
                'billboard_id' => (int) $id,
                'billboard_title' => $rows->first()['billboard_title'],
                'payments_counted' => $rows->count(),
                'bookings_counted' => $rows->pluck('booking_id')->unique()->count(),
                'collected' => round((float) $rows->sum('amount'), 2),
                'platform_cut' => round((float) $rows->sum('platform_cut'), 2),
                'earnings' => round((float) $rows->sum('owner_earning'), 2),
                'paid_out' => round((float) $rows->where('payout_status', 'paid_out')->sum('owner_earning'), 2),
                'ready_for_payout' => round((float) $rows->where('payout_status', 'ready')->sum('owner_earning'), 2),
                'awaiting_verification' => round((float) $rows->where('payout_status', 'awaiting_verification')->sum('owner_earning'), 2),
                'in_progress' => round((float) $rows->where('payout_status', 'in_progress')->sum('owner_earning'), 2),
            ])
            ->sortByDesc('earnings')
            ->values()
            ->all();

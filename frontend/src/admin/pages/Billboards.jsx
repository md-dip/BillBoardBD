...
export default function AdminBillboards() {
  ...
  function loadRequests() {
    return api.get('/admin/billboards', { params: { listing_status: 'pending_review' } })
  ...
  async function handleApprove(id) {
    ...
    setActingId(id);
    try {
      await api.patch(`/admin/billboards/${id}/approve`);
      load();
    } catch (err) {
      setError(err.response?.data?.message || 'Could not approve this board.');
    } finally {
      setActingId(null);
    }
  }
  ...
  async function submitReject(id) {
    ...
    setActingId(id);
    try {
      await api.patch(`/admin/billboards/${id}/reject`, { rejection_reason: rejectReason });
      setRejectingId(null);
      setRejectReason('');
      load();
    } catch (err) {
      setError(err.response?.data?.message || 'Could not reject this board.');
    } finally {
      setActingId(null);
    }
  }
  ...
  return (
    <AdminShell title="Billboards">
      <div className="admin-billboards-tabs">
        <button
          type="button"
          className={`admin-billboards-tab-all ${tab === 'all' ? 'admin-billboards-tab-active' : ''}`}
          onClick={() => setTab('all')}
        >
          All billboards
        </button>
        <button
          type="button"
          className={`admin-billboards-tab-requests ${tab === 'requests' ? 'admin-billboards-tab-active' : ''}`}
                  ...
                  {billboards.map((b) => {
                    ...
                    return (
                      <tr key={b.id}>
                        ...
                        <td>
                          <span className={`admin-billboards-listing-badge-${(b.listing_status || 'approved').replace(/_/g, '-')}`}>
      ...
      {tab === 'requests' && (
        <>
          ...
          {loading ? (
            ...
            <div className="admin-billboards-table-card admin-billboards-requests-empty">
              No listing requests waiting for review.
            </div>
          ) : (
                ...
                <div className="admin-billboards-request-card" key={b.id}>
                  ...
                  <div className="admin-billboards-request-body">
                    ...
                    <div className="admin-billboards-request-meta">
                      ...
                      <span>{b.pricing_mode === 'monthly' ? `${formatBDT(b.monthly_rate)}/mo` : `${formatBDT(b.daily_rate)}/day`}</span>
                      <span>Permit expiry: {b.permit_expiry_date ? b.permit_expiry_date.slice(0, 10) : 'N/A'}</span>
                      {b.submitted_at && <span>Submitted: {b.submitted_at.slice(0, 10)}</span>}
                    </div>
                    {b.permit_document_url && (
                      <a className="admin-billboards-permit-doc-link" href={b.permit_document_url} target="_blank" rel="noreferrer">
                  ...
                  <div className="admin-billboards-request-actions">
                    {rejectingId === b.id ? (
                      <div className="admin-billboards-reject-box">
                        <input
                          className="admin-billboards-reject-reason-input"
                          placeholder="Reason for rejection"
                          value={rejectReason}
                          onChange={(e) => setRejectReason(e.target.value)}
                        />
                        <div className="admin-billboards-reject-box-actions">
                          <button
                            className="admin-billboards-confirm-reject-btn"
                            disabled={actingId === b.id || rejectReason.trim().length < 3}
                            onClick={() => submitReject(b.id)}
                          >
                            Confirm reject
                          </button>
                          <button className="admin-billboards-cancel-reject-btn" onClick={() => { setRejectingId(null); setRejectReason(''); }}>
                            Cancel
                          </button>
                        </div>
                      </div>
                      ...
                      <>
                        <button
                          className="admin-billboards-approve-listing-btn"
                        ...
                        <button
                          className="admin-billboards-reject-listing-btn"
